...
Forwarder::Forwarder()
    : m_unsolicitedDataPolicy(new fw::DefaultUnsolicitedDataPolicy()),
      m_fib(m_nameTree), m_pit(m_nameTree), m_measurements(m_nameTree),
  ...
  m_faceTable.afterAdd.connect([this](Face &face) {
    ...
    face.afterReceiveNack.connect([this, &face](const lp::Nack &nack) {
...
void Forwarder::onIncomingInterest(Face &inFace, const Interest &interest) {
  ...
  bool isViolatingLocalhost =
      inFace.getScope() == ndn::nfd::FACE_SCOPE_NON_LOCAL &&
  ...
  if (!pitEntry->hasInRecords()) {
    m_cs.find(
        interest, bind(&Forwarder::onContentStoreHit, this, ref(inFace),
                       pitEntry, _1, _2),
...
void Forwarder::onInterestLoop(Face &inFace, const Interest &interest) {
  ...
  if (inFace.getLinkType() != ndn::nfd::LINK_TYPE_POINT_TO_POINT) {
    NFD_LOG_DEBUG("onInterestLoop face=" << inFace.getId() << " interest="
...
void Forwarder::onContentStoreHit(const Face &inFace,
  ...
  this->onOutgoingData(contentName, data,
...
void Forwarder::onInterestReject(const shared_ptr<pit::Entry> &pitEntry) {
...
void Forwarder::onInterestUnsatisfied(const shared_ptr<pit::Entry> &pitEntry) {
...
void Forwarder::onInterestFinalize(
    const shared_ptr<pit::Entry> &pitEntry, bool isSatisfied,
...
void Forwarder::onDataUnsolicited(Face &inFace, const Data &data) {
  ...
  fw::UnsolicitedDataDecision decision =
      m_unsolicitedDataPolicy->decide(inFace, data);
...
void Forwarder::onOutgoingData(const Name &name, const Data &data,
                               Face &outFace) {
...
void Forwarder::onIncomingNack(Face &inFace, const lp::Nack &nack) {
  // receive Nack
  ...
  // if multi-access or ad hoc face, drop
  if (inFace.getLinkType() != ndn::nfd::LINK_TYPE_POINT_TO_POINT) {
    NFD_LOG_DEBUG("onIncomingNack face="
                  << inFace.getId() << " nack=" << nack.getInterest().getName()
  ...
  if (nack.getInterest().getNonce() != outRecord->getLastNonce()) {
    NFD_LOG_DEBUG("onIncomingNack face=" << inFace.getId() << " nack="
...
void Forwarder::onOutgoingNack(const shared_ptr<pit::Entry> &pitEntry,
  ...
  if (inRecord == pitEntry->in_end()) {
    NFD_LOG_DEBUG("onOutgoingNack face=" << outFace.getId() << " nack="
...
void Forwarder::onDroppedInterest(Face &outFace, const Interest &interest) {
  m_strategyChoice.findEffectiveStrategy(interest.getName())
      .onDroppedInterest(outFace, interest);
}
...
void Forwarder::setUnsatisfyTimer(const shared_ptr<pit::Entry> &pitEntry) {
  ...
  time::nanoseconds lastExpiryFromNow =
      lastExpiry - time::steady_clock::now() + time::nanoseconds(0);
...
void Forwarder::cancelUnsatisfyAndStragglerTimer(pit::Entry &pitEntry) {
  scheduler::cancel(pitEntry.m_unsatisfyTimer);
  scheduler::cancel(pitEntry.m_stragglerTimer);
}
